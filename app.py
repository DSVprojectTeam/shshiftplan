...
DEFAULT_SCHEDULE_COLS = ["A,C,D,E:AI",
                         "A,C,D,E:AH", "A,C,D,E:AG", "A,C,D,E:AF"]
...
WORKING_DAYS = ["EM_O", "AP_O", "AM_O", "EM_C", "AP_C",
                "AM_C", "EM_P", "AP_P", "AM_P", "AP_W", "EM_W", "AM_W"]
...
def load_schedule_data(hub=None, selected_date=None):
    ...
    shift_counts = df[today_str].value_counts(
    ).to_dict() if today_str in df.columns else {}
...
def dayoff_show_summary():
    ...
    for sheet_name in ALL_MONTHS:
        ...
        df["SL"] = df.iloc[:, 4:35].apply(
            lambda row: (row == "SL").sum(), axis=1)
        df["VL"] = df.iloc[:, 4:35].apply(
            lambda row: (row == "VL").sum(), axis=1)
        df["O"] = df.iloc[:, 4:35].apply(
            lambda row: (row == "O").sum(), axis=1)
        df["DD"] = df.iloc[:, 4:35].apply(
            lambda row: (row == "DD").sum(), axis=1)
    ...
    summary_df = pd.DataFrame.from_dict(
        yearly_summary, orient='index').reset_index()
...
def get_availability():
    ...
    unavailable_statuses = {"RD", "VL", "PH", "EL",
                            "SL", "ML", "PL", "BD", "O", "OS", "DD"}
    ...
    for shift in manila_shifts:
        if shift in phone_statuses:
            if shift == 'EM_P':
                region_counts['EMEA']['phone'] += 1
            elif shift == 'AP_P':
                region_counts['APAC']['phone'] += 1
            elif shift == 'AM_P':
                region_counts['AMER']['phone'] += 1
        elif shift in chat_statuses:
            if shift == 'EM_C':
                region_counts['EMEA']['chat'] += 1
            elif shift == 'AP_C':
                region_counts['APAC']['chat'] += 1
            elif shift == 'AM_C':
                region_counts['AMER']['chat'] += 1
        elif shift in offline_statuses:
            if shift == 'EM_O':
                region_counts['EMEA']['offline'] += 1
            elif shift == 'AP_O':
                region_counts['APAC']['offline'] += 1
            elif shift == 'AM_O':
                region_counts['AMER']['offline'] += 1
        elif shift in weekender_statuses:
            if shift == 'EM_W':
                region_counts['EMEA']['weekenders'] += 1
            elif shift == 'AP_W':
                region_counts['APAC']['weekenders'] += 1
            elif shift == 'AM_W':
                region_counts['AMER']['weekenders'] += 1
...
def download_schedule():
    selected_month = request.args.get(
        'month') or datetime.today().strftime("%B")
    df = read_schedule_sheet(
        selected_month, usecols_list=DEFAULT_DOWNLOAD_COLS)
